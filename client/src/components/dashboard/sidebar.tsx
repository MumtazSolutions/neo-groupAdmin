...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
            ...
            {teamItems.map((item) => {
              ...
              return (
                <Link 
                  key={item.href} 
                  href={item.href} 
                  className="text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 group flex items-center px-3 py-2 text-sm font-medium rounded-lg transition-colors"
                  onClick={isMobile ? onClose : undefined}
                >
                  <Icon className="text-gray-400 mr-3 h-4 w-4 flex-shrink-0" />
                  <span className="truncate">{item.label}</span>
                </Link>
              );
            })}
  ...
  return (
    <>
      ...
      {/* Desktop Sidebar */}
      <div className={`
        fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-gray-800 shadow-lg transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 lg:block
